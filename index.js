...
function zip() {
  ...
  function getTimestamp() {
    ...
    let mm = new Intl.DateTimeFormat('en', { month: 'numeric' }).format(now)
    if (mm < 10) mm = `0${mm}`
    let dd = new Intl.DateTimeFormat('en', { day: '2-digit' }).format(now)
    let hour = new Intl.DateTimeFormat('en', { hour: 'numeric', hour12: false }).format(now)
    let min = new Intl.DateTimeFormat('en', { minute: 'numeric' }).format(now)
    let sec = new Intl.DateTimeFormat('en', { second: 'numeric' }).format(now)
